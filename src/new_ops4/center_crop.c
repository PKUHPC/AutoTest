...
#define center_crop3d_kernel(typename)                                                     \
...
#define center_crop4d_kernel(typename)                                                         \
    typename *in_data = aitisa_tensor_data(input);                                                    \
    typename *out_data = aitisa_tensor_data(*output);                                                 \
    int64_t in_channel_offset = ncol * nrow;                                                          \
    int64_t out_channel_offset = target_h * target_w;                                                 \
...
Status aitisa_center_crop(const Tensor input, int target_h, int target_w, Tensor *output)
{
    ...
    if (ndim == 3) // [C, H, W]
    {
        ...
        int64_t start_i = (nrow - target_h) / 2;
        int64_t start_j = (ncol - target_w) / 2;
        ...
        *output = new_tensor;
        ...
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, center_crop3d_kernel);
    }
    else if (ndim == 4) // [N, C, H, W]
    {
        ...
        *output = new_tensor;
        int64_t size = aitisa_tensor_size(input);
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, center_crop4d_kernel);
    }
    else
    {
        status = STATUS_TYPE_MISMATCH;
