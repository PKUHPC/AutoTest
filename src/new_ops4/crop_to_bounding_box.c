#include "src/new_ops4/crop_to_bounding_box.h"
#include "src/core/dispatch.h"

#define crop_to_bounding_box3d_kernel(typename)                                                     \
...
#define crop_to_bounding_box4d_kernel(typename)                                                         \
    typename *in_data = aitisa_tensor_data(input);                                                    \
    typename *out_data = aitisa_tensor_data(*output);                                                 \
    int64_t in_channel_offset = ncol * nrow;                                                          \
    int64_t out_channel_offset = target_h * target_w;                                                 \
...
Status aitisa_crop_to_bounding_box(const Tensor input, int offset_h, int offset_w, int target_h, int target_w, Tensor *output)
{
    ...
    if (ndim == 3) // [C, H, W]
    {
        ...
        int64_t start_i = offset_h;
        int64_t start_j = offset_w;
        ...
        *output = new_tensor;
        ...
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, crop_to_bounding_box3d_kernel);
    }
    else if (ndim == 4) // [N, C, H, W]
    {
        ...
        *output = new_tensor;
        int64_t size = aitisa_tensor_size(input);
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, crop_to_bounding_box4d_kernel);
    }
    else
    {
        status = STATUS_TYPE_MISMATCH;
