...
static Status exp_create_output(const Tensor input, Tensor* output) {
  Status status;
...
Status aitisa_exp(const Tensor input, Tensor* output) {
  Status status = STATUS_SUCCESS;
  CHECK_STATUS(exp_create_output(input, output));
  DataType dtype = aitisa_tensor_data_type(input);
  AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, exp_kernel);
  return status;
}