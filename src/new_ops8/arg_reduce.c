...
#define arg_value_kernel(typename, greater)                     \
  typename* in_data = (typename*)aitisa_tensor_data(input);     \
...
Status aitisa_min(const Tensor input, const int64_t dim, const int keepdim,
                  Tensor* output) {
  ...
  bool greater = false;
  AITISA_DISPATCH_ALL_TYPES_RETURN(prods_dtype, arg_value_kernel, greater);
  if (!keepdim) {
    aitisa_squeeze(*output, NULL, 0, output);
  }
  return status;
...
Status aitisa_max(const Tensor input, const int64_t dim, const int keepdim,
                  Tensor* output) {
