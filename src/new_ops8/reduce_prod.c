...
static Status reduce_prod_create_output(const Tensor input, const int64_t* dims,
  ...
  status =
      aitisa_full(dtype, device, new_tensor_dims, input_ndim, 1, &new_tensor);
  *output = new_tensor;
  return status;
}

#define reduce_prod_kernel(typename, permuted_strides_in,                  \
                           permuted_strides_out, permuted_dims, num_items) \
...
Status aitisa_reduce_prod(const Tensor input, const int64_t* dims,
                          const int64_t dims_length, const int keepdim,
                          Tensor* output) {

  Status status = STATUS_SUCCESS;
  DataType prods_dtype = aitisa_tensor_data_type(input);
  CHECK_STATUS(reduce_prod_create_output(input, dims, dims_length, output));
  ...
  AITISA_DISPATCH_ALL_TYPES_RETURN(prods_dtype, reduce_prod_kernel,
