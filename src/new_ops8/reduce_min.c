...
static Status reduce_min_create_output(const Tensor input, const int64_t* dims,
...
#define reduce_min_kernel(typename, permuted_strides_in, permuted_strides_out, \
...
Status aitisa_reduce_min(const Tensor input, const int64_t* dims,
                         const int64_t dims_length, const int keepdim,
                         Tensor* output) {

  Status status = STATUS_SUCCESS;
  DataType prods_dtype = aitisa_tensor_data_type(input);
  CHECK_STATUS(reduce_min_create_output(input, dims, dims_length, output));
  ...
  AITISA_DISPATCH_ALL_TYPES_RETURN(prods_dtype, reduce_min_kernel,
