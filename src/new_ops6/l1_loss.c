...
static Status l1_loss_create_output(const Tensor input, Tensor* output) {
  Status status;
...
Status aitisa_l1_loss(const Tensor input, const Tensor target,
                      const Tensor weight, const int reduction,
                      Tensor* output) {

  Status status = STATUS_SUCCESS;
  l1_loss_create_output(input, output);
  aitisa_sub(input, target, output);
  aitisa_abs(*output, output);
  if (weight) {
