...
#define random_vflip3d_kernel(typename)                                         \
    ...
    {                                                                           \
        ...
        {                                                                       \
            ...
            {                                                                   \
                ...
                int64_t in_idx = channel_offset + (nrow - 1 - i) * ncol + ncol; \
...
#define random_vflip4d_kernel(typename)                                             \
    typename *in_data = aitisa_tensor_data(input);                                  \
    typename *out_data = aitisa_tensor_data(*output);                               \
...
Status aitisa_random_vertical_flip(const Tensor input, const float prob, const int seed, Tensor *output)
{
    // check params
    if (prob < 0 || prob > 1)
    {
    ...
    if (ndim == 3) // [C, H, W]
    {
        ...
        *output = new_tensor;
        ...
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, random_vflip3d_kernel);
    }
    else if (ndim == 4) // [N, C, H, W]
    {
        ...
        *output = new_tensor;
        int64_t size = aitisa_tensor_size(input);
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, random_vflip4d_kernel);
    }
    else
    {
