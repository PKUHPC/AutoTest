...
#define hflip2d_kernel(typename)                             \
    ...
    {                                                        \
        ...
        {                                                    \
            ...
            int64_t in_idx = i * ncol + (ncol - 1 - j);      \
            out_data[out_idx] = (typename)(in_data[in_idx]); \
        }                                                    \
    }

#define hflip3d_kernel(typename)                                             \
    typename *in_data = aitisa_tensor_data(input);                           \
    typename *out_data = aitisa_tensor_data(*output);                        \
...
Status aitisa_horizontal_flip(const Tensor input, Tensor *output)
{
    ...
    if(ndim==2)
    {
        ...
        *output = new_tensor;
        ...
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, hflip2d_kernel);
    }
    else if(ndim==3)
    {
        int nchannel = dims[0];
        ...
        *output = new_tensor;
        int64_t size = aitisa_tensor_size(input);
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, hflip3d_kernel);
    }
    else
    {
