...
#define random_hue3d_kernel(typename)                                                            \
    ...
    {                                                                                            \
        ...
        {                                                                                        \
            ...
            if (fabs(delta - 0.0) < 0.0001)                                                      \
            {                                                                                    \
                out_data[idx] = 0;                                                               \
            }                                                                                    \
            else if (fabs(Cmax - R) < 0.0001)                                                    \
            {                                                                                    \
                if (G >= B)                                                                      \
                {                                                                                \
            ...
            else if (fabs(Cmax - G) < 0.0001)                                                    \
            {                                                                                    \
                out_data[idx] = 60.0 * ((B - R) / delta + 2.0);                                  \
            }                                                                                    \
            else if (fabs(Cmax - B) < 0.0001)                                                    \
            {                                                                                    \
                out_data[idx] = 60.0 * ((R - G) / delta + 4.0);                                  \
            }                                                                                    \
            if (fabs(Cmax - 0.0) < 0.0001)                                                       \
            {                                                                                    \
...
#define random_hue4d_kernel(typename)                                                                \
    typename *in_data = aitisa_tensor_data(input);                                                   \
    typename *out_data = aitisa_tensor_data(*output);                                                \
    int64_t channel_offset = ncol * nrow;                                                            \
...
Status aitisa_random_hue(const Tensor input, double lower, double upper, Tensor *output)
{
    int64_t *dims = aitisa_tensor_dims(input);
    int64_t ndim = aitisa_tensor_ndim(input);
    Status status = STATUS_SUCCESS;
    //srand(0);
    double hue_factor = (rand() / (double)RAND_MAX) * (upper - lower) + lower;
    // printf("%f", hue_factor);
    if (ndim == 3) // [C, H, W]
    {
        int nchannel = dims[0];
        ...
        *output = new_tensor;
        ...
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, random_hue3d_kernel);
    }
    else if (ndim == 4) // [N, C, H, W]
    {
        ...
        *output = new_tensor;
        int64_t size = aitisa_tensor_size(input);
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, random_hue4d_kernel);
    }
    else
    {
        status = STATUS_TYPE_MISMATCH;
