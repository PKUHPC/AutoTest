#include "src/new_ops3/adjust_saturation.h"
...
#define adjust_saturation3d_kernel(typename)                                                             \
...
#define adjust_saturation4d_kernel(typename)                                                                 \
    typename *in_data = aitisa_tensor_data(input);                                                           \
    typename *out_data = aitisa_tensor_data(*output);                                                        \
    int64_t channel_offset = ncol * nrow;                                                                    \
...
Status aitisa_adjust_saturation(const Tensor input, double saturation_factor, Tensor *output)
{
    ...
    if (ndim == 3) // [C, H, W]
    {
        ...
        *output = new_tensor;
        ...
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, adjust_saturation3d_kernel);
    }
    else if (ndim == 4) // [N, C, H, W]
    {
        ...
        *output = new_tensor;
        int64_t size = aitisa_tensor_size(input);
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, adjust_saturation4d_kernel);
    }
    else
    {
        status = STATUS_TYPE_MISMATCH;
