...
#define random_contrast3d_kernel(typename)                                                                               \
...
#define random_contrast4d_kernel(typename)                                                                                   \
    typename *in_data = aitisa_tensor_data(input);                                                                                  \
    typename *out_data = aitisa_tensor_data(*output);                                                                               \
    double sum = 0.0;                                                                                                               \
...
Status aitisa_random_contrast(const Tensor input, double lower, double upper, Tensor *output)
{
    int64_t *dims = aitisa_tensor_dims(input);
    int64_t ndim = aitisa_tensor_ndim(input);
    Status status = STATUS_SUCCESS;
    double contrast_factor = (rand() / (double)RAND_MAX) * (upper - lower) + lower;
    if (ndim == 3) // [C, H, W]
    {
        int nchannel = dims[0];
        int nrow = dims[1];
        ...
        *output = new_tensor;
        ...
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, random_contrast3d_kernel);
    }
    else if (ndim == 4) // [N, C, H, W]
    {
        ...
        *output = new_tensor;
        int64_t size = aitisa_tensor_size(input);
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, random_contrast4d_kernel);
    }
    else
    {
        status = STATUS_TYPE_MISMATCH;
