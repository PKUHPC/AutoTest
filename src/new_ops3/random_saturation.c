...
#define random_saturation3d_kernel(typename)                                                             \
    ...
    {                                                                                                    \
        ...
        {                                                                                                \
            ...
            out_data[channel_offset * 1 + idx] = out_data[channel_offset * 1 + idx] * saturation_factor; \
            if (out_data[channel_offset * 1 + idx] > 1.0)                                                \
            {                                                                                            \
                out_data[channel_offset * 1 + idx] = 1.0;                                                \
            }                                                                                            \
            if (out_data[channel_offset * 1 + idx] < 0.0)                                                \
            {                                                                                            \
                out_data[channel_offset * 1 + idx] = 0.0;                                                \
            }                                                                                            \
...
#define random_saturation4d_kernel(typename)                                                                 \
    typename *in_data = aitisa_tensor_data(input);                                                           \
    typename *out_data = aitisa_tensor_data(*output);                                                        \
    int64_t channel_offset = ncol * nrow;                                                                    \
...
Status aitisa_random_saturation(const Tensor input, double lower, double upper, Tensor *output)
{
    int64_t *dims = aitisa_tensor_dims(input);
    int64_t ndim = aitisa_tensor_ndim(input);
    Status status = STATUS_SUCCESS;
    //srand(0);
    double saturation_factor = (rand() / (double)RAND_MAX) * (upper - lower) + lower;
    // printf("%f", hue_factor);
    if (ndim == 3) // [C, H, W]
    {
        int nchannel = dims[0];
        ...
        *output = new_tensor;
        ...
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, random_saturation3d_kernel);
    }
    else if (ndim == 4) // [N, C, H, W]
    {
        ...
        *output = new_tensor;
        int64_t size = aitisa_tensor_size(input);
        AITISA_DISPATCH_ALL_TYPES_RETURN(dtype, random_saturation4d_kernel);
    }
    else
    {
        status = STATUS_TYPE_MISMATCH;
