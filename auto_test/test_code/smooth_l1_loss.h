...
class SmoothL1LossInput : public Binary_Input {
 public:
  SmoothL1LossInput() = default;
  SmoothL1LossInput(int64_t ndim1, std::vector<int64_t> dims1, int dtype1,
                    int device1, void* data1, unsigned int len1, int64_t ndim2,
                    std::vector<int64_t> dims2, int dtype2, int device2,
                    void* data2, unsigned int len2, std::vector<int> weight,
  ...
  SmoothL1LossInput(SmoothL1LossInput&& input) noexcept
  ...
  ~SmoothL1LossInput() override { delete[] weight_; }
  SmoothL1LossInput& operator=(SmoothL1LossInput&& right) noexcept {
...
class SmoothL1LossTest : public ::testing::Test {
 public:
  SmoothL1LossTest() {
    fetch_test_data("smooth_l1_loss", smooth_l1_loss_inputs,
                    smooth_l1_loss_inputs_name);
  }
  ~SmoothL1LossTest() override = default;

  int fetch_test_data(const char* path, std::vector<SmoothL1LossInput>& inputs,
    ...
    try {
      ...
      for (int i = 0; i < count; ++i) {
        ...
        if (!setting.lookupValue("beta", beta)) {
          std::cerr << "Setting \"beta\" do not exist in test index "
                    << test_index << " from " << path << " !\n"
                    << std::endl;
          continue;
        }
        ...
        SmoothL1LossInput tmp(ndim1, dims1, dtype1, device1, nullptr, len1,
                              ndim2, dims2, dtype2, device2, nullptr, len2,
                              weight, reduction, beta);
  ...
  using InputType = SmoothL1LossInput;
  using UserInterface = InterfaceType;
  // inputs
  std::vector<SmoothL1LossInput> smooth_l1_loss_inputs;
  std::vector<std::string> smooth_l1_loss_inputs_name;
  std::map<std::string, int> test_case = {{"smooth_l1_loss", 0}};
};
TYPED_TEST_CASE_P(SmoothL1LossTest);

TYPED_TEST_P(SmoothL1LossTest, TwoTests) {
  ...
  time_map m;
  auto test = [&m](std::vector<SmoothL1LossInput>&& inputs,
                   std::vector<std::string>&& inputs_name,
    ...
    for (int i = 0; i < inputs.size(); i++) {
      ...
      for (int n = 0; n < loop; n++) {
        ...
        aitisa_smooth_l1_loss(aitisa_tensor1, aitisa_tensor2, aitisa_weight,
        ...
        user_result = UserFuncs::user_smooth_l1_loss(
        ...
        torch_result = torch::smooth_l1_loss(
  ...
  if (this->smooth_l1_loss_inputs.size()) {
    test(std::move(this->smooth_l1_loss_inputs),
         std::move(this->smooth_l1_loss_inputs_name), "smooth_l1_loss",
         this->test_case["smooth_l1_loss"]);
#ifdef AITISA_API_GENERATE_FIGURE
    draw_fig_fun(m, "smooth_l1_loss");
#endif
  } else
    FAIL() << "No input test case.";
}
REGISTER_TYPED_TEST_CASE_P(SmoothL1LossTest, TwoTests);

#define REGISTER_SMOOTHL1LOSS(SMOOTHL1LOSS)                                    \
  class SmoothL1Loss : public Basic {                                          \
   public:                                                                     \
    static UserTensor user_smooth_l1_loss(UserTensor input, UserTensor target, \
                                          const int* weight,                   \
  ...
  namespace aitisa_api {                                                       \
  INSTANTIATE_TYPED_TEST_CASE_P(aitisa_api, SmoothL1LossTest, SmoothL1Loss);   \
  }
