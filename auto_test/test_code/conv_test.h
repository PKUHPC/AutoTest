...
class Conv2dTest : public ::testing::Test {
 public:
  ...
  std::string input0_name =
      "Random float of CPU with InputDims{6,32,124,128}, "
      "FilterDims{64,32,2,2}, stride{2,2}, padding{0,0}, dilation{1,1}";
  std::string input1_name =
      "Random float of CPU with InputDims{7,16,100,100}, "
      "FilterDims{32,16,3,3}, stride{3,3}, padding{1,0}, dilation{2,2}";
...
TYPED_TEST_P(Conv2dTest, TwoTests) {
  ...
  for (int i = 0; i < this->ninput; i++) {
    ...
    AITISA_DataType aitisa_dtype1 =
        aitisa_int_to_dtype(this->input[i]->dtype1());
    AITISA_DataType aitisa_dtype2 =
        aitisa_int_to_dtype(this->input[i]->dtype2());
    ...
    aitisa_create(aitisa_dtype1, aitisa_device1, this->input[i]->dims1(),
                  this->input[i]->ndim1(), (void*)(this->input[i]->data1()),
                  this->input[i]->len1(), &aitisa_tensor1);
    aitisa_create(aitisa_dtype2, aitisa_device2, this->input[i]->dims2(),
                  this->input[i]->ndim2(), (void*)(this->input[i]->data2()),
                  this->input[i]->len2(), &aitisa_tensor2);
    gettimeofday(&aitisa_start, NULL);
    aitisa_conv2d(aitisa_tensor1, aitisa_tensor2, this->input[i]->stride(), 2,
                  this->input[i]->padding(), 2, this->input[i]->dilation(), 2,
                  this->input[i]->groups(), &aitisa_result);
    ...
    UserDataType user_dtype1 =
        UserFuncs::user_int_to_dtype(this->input[i]->dtype1());
    UserDataType user_dtype2 =
        UserFuncs::user_int_to_dtype(this->input[i]->dtype2());
    UserDevice user_device1 =
        UserFuncs::user_int_to_device(this->input[i]->device1());
    UserDevice user_device2 =
        UserFuncs::user_int_to_device(this->input[i]->device2());
    ...
    UserFuncs::user_conv2d(user_tensor1, user_tensor2, this->input[i]->stride(),
                           2, this->input[i]->padding(), 2,
                           this->input[i]->dilation(), 2,
                           this->input[i]->groups(), &user_result);
