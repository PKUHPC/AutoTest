...
class Batchnorm_Input : public Unary_Input {
 public:
  ...
  virtual ~Batchnorm_Input() { delete[] param_dims_; }
 ...
 public:
  BatchnormTest()
      : input0(/*ndim*/ 4, /*dims*/ {2, 3, 2, 2}, /*dtype=float*/ 8,
               /*device=cpu*/ 0, /*data*/ nullptr, /*len*/ 0, 1, 1e-5, 1, {3},
               1, 0.5, 0),
        input1(/*ndim*/ 4, /*dims*/ {200, 300, 20, 20}, /*dtype=float*/ 8,
               /*device=cpu*/ 0, /*data*/ nullptr, /*len*/ 0, 1, 1e-5, 1, {300},
               1, 0.5, 0) {
    input[0] = &input0;
    input[1] = &input1;
    ninput = 2;
  ...
  std::string input0_name =
      "Random float of CPU with InputDims{2, 3, 2, 2}, axis{1}, epsilon{1e-5}, "
      "param_ndim{1}, param_dims{3}, value{1}, mean{0.5}, var{0}";
  std::string input1_name =
      "Random float of CPU with InputDims{200, 300, 20, 20}, axis{1}, "
      "epsilon{1e-5}, param_ndim{1}, param_dims{300}, value{1}, mean{0.5}, "
      "var{0}";
...
TYPED_TEST_P(BatchnormTest, TwoTests) {
  ...
  for (int i = 0; i < this->ninput; i++) {
    ...
    aitisa_create(aitisa_dtype, aitisa_device, this->input[i]->param_dims(),
                  this->input[i]->param_ndim(), NULL, 0, &mean);
    full_float(mean, this->input[i]->mean());

    aitisa_create(aitisa_dtype, aitisa_device, this->input[i]->param_dims(),
                  this->input[i]->param_ndim(), NULL, 0, &variance);
    full_float(variance, this->input[i]->var());

    aitisa_create(aitisa_dtype, aitisa_device, this->input[i]->param_dims(),
                  this->input[i]->param_ndim(), NULL, 0, &scale);
    full_float(scale, 1);

    aitisa_create(aitisa_dtype, aitisa_device, this->input[i]->param_dims(),
                  this->input[i]->param_ndim(), NULL, 0, &bias);
    full_float(bias, 0);

    gettimeofday(&aitisa_start, NULL);

    aitisa_batch_norm(aitisa_tensor, this->input[i]->axis(), scale, bias, mean,
                      variance, this->input[i]->epsilon(), &aitisa_result);
    ...
    UserTensor bn_mean = hice::full(param_dims, this->input[i]->mean(),
                                    device(hice::kCPU).dtype(hice::kFloat));
    UserTensor bn_var = hice::full(param_dims, this->input[i]->var(),
                                   device(hice::kCPU).dtype(hice::kFloat));

    UserFuncs::user_create(user_dtype, user_device, this->input[i]->dims(),
                           this->input[i]->ndim(), NULL, this->input[i]->len(),
                           &user_result);

    UserFuncs::user_batchnorm(
        user_tensor, this->input[i]->axis(), bn_scale, bn_bias, running_mean,
        running_var, this->input[i]->epsilon(), user_result, bn_mean, bn_var);
