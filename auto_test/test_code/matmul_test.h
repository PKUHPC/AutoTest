...
class MatmulTest : public ::testing::Test {
 public:
  ...
  Binary_Input input5;
  Binary_Input input6;
  Binary_Input* input[7] = {&input0, &input1, &input2, &input3,
                            &input4, &input5, &input6};
  std::string input0_name = "Natural Float CPU with Dims{10} and Dims{10}";
  std::string input1_name =
      "Random Double CPU with Dims{199,202} and Dims{202,201}";
  ...
  std::string input4_name =
      "Natural Float CPU with Dims{3} and Dims{2,2,4,3,2}";
  std::string input5_name =
      "Natural Float CPU with Dims{2,2,4,2,3} and Dims{3}";
  std::string input6_name =
      "Natural Float CPU with Dims{2,4,3} and Dims{3,2,3,2}";
  std::string* input_name[7] = {&input0_name, &input1_name, &input2_name,
                                &input3_name, &input4_name, &input5_name,
                                &input6_name};
  int ninput = 7;
};
...
TYPED_TEST_P(MatmulTest, SevenTests) {
  ...
  for (int i = 0; i < this->ninput; i++) {
    ...
    user_end = std::clock();
    user_time =
        1000.0 * (user_end - user_start) / static_cast<double>(CLOCKS_PER_SEC);
