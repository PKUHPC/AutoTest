...
class Unary_Input {
 public:
  ...
  Unary_Input(int64_t ndim, int64_t* dims, int dtype, int device, void* data,
              unsigned int len);
  Unary_Input(int64_t ndim, std::vector<int64_t> dims, int dtype, int device,
  ...
  void set_data(void* data, unsigned int len) {
    data_ = data;
    len_ = len;
  }
...
class Binary_Input {
 public:
  ...
  Binary_Input(int64_t ndim1, int64_t* dims1, int dtype1, int device1,
...
class Result {
 public:
  ...
  int* count() { return count_; }
  virtual void set_result(int64_t ndim, int64_t* dims, int dtype, int device,
                          void* data, unsigned int len);
...
class Sample {
 public:
  ...
  Sample(Sample<InputType>& sample)
      : input_(sample.input()), result_(sample.result()) {}
  Sample(Sample<InputType>&& sample)
      : input_(sample.input()), result_(sample.result()) {}
  virtual ~Sample() {}
  ...
  void set_result(int64_t ndim, int64_t* dims, int dtype, int device,
                  void* data, unsigned int len) {
...
Sample<Binary_Input> get_binary_sample(int sample_num) {
  ...
  aitisa_create(dtype1, device1, input.dims1(), input.ndim1(), input.data1(),
                input.len1(), &tensor_in1);
  aitisa_create(dtype2, device2, input.dims2(), input.ndim2(), input.data2(),
                input.len2(), &tensor_in2);
  TestType::aitisa_kernel(tensor_in1, tensor_in2, &tensor_out);
  aitisa_resolve(tensor_out, &dtype_out, &device_out, &dims_out, &ndim_out,
                 &data_out, &len_out);
  dtype_out_num = aitisa_dtype_to_int(dtype_out);
  device_out_num = aitisa_device_to_int(device_out);
  sample.set_result(ndim_out, dims_out, dtype_out_num, device_out_num, data_out,
                    len_out);
