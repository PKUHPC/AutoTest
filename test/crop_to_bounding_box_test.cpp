...
void crop_to_bounding_box_assign_float(Tensor t) {
...
TEST(CropToBox, Float3d) {
  Tensor input;
  DataType dtype = kFloat;
  Device device = {DEVICE_CPU, 0};
  int64_t dims[3] = {3, 5, 5};
  aitisa_create(dtype, device, dims, 3, NULL, 0, &input);
  crop_to_bounding_box_assign_float(input);
  // tensor_printer2d(input);

  Tensor output;
  aitisa_crop_to_bounding_box(input, 1, 1, 3, 3, &output);
...
TEST(CropToBox, Float4d) {
  ...
  Tensor output;
  aitisa_crop_to_bounding_box(input, 1, 1, 2, 2, &output);
  // tensor_printer2d(output);
