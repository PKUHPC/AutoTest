...
void square_assign_int32(Tensor t) {
...
void square_assign_float(Tensor t) {
  int64_t size = aitisa_tensor_size(t);
...
TEST(Square, Float) {
  ...
  square_assign_float(input);

  Tensor output;
  aitisa_square(input, &output);

  float* out_data = (float*)aitisa_tensor_data(output);
  float test_data[] = {2.25, 1, 0.25, 0, 0.25, 1};
...
TEST(Square, Int32) {
  ...
  int32_t test_data[] = {16, 9, 4, 1, 0, 1};
  int64_t size = aitisa_tensor_size(input);
  for (int64_t i = 0; i < size; i++) {
