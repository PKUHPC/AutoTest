...
void adjust_saturation_assign_float(Tensor t) {
...
TEST(AdjustHue, Float3d) {
  ...
  adjust_saturation_assign_float(input);
  // tensor_printer2d(input);
  double adjust_factor = 1.2;

  Tensor output;
  aitisa_adjust_saturation(input, adjust_factor, &output);
  // tensor_printer2d(output);

  float* out_data = (float*)aitisa_tensor_data(output);
  float test_data[] = {0, 0, 0, 0.6, 1.6, 2.6, 12, 13, 14, 15, 16, 17, 12, 13, 14, 15, 16, 17};
