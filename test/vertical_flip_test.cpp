...
void vflip_assign_float(Tensor t)
{
...
namespace aitisa_api
{
    namespace
    {
        ...
        TEST(vflip, Float2d)
        {
            ...
            vflip_assign_float(input);
            // tensor_printer2d(input);

            Tensor output;
            aitisa_vertical_flip(input, &output);
            // tensor_printer2d(output);

            float *out_data = (float *)aitisa_tensor_data(output);
            float test_data[] = {0, 0.1, 0.2, -0.3, -0.2, -0.1};
        ...
        TEST(vflip, Float3d)
        {
            ...
            float *out_data = (float *)aitisa_tensor_data(output);
            float test_data[] = {0, 0.1, 0.2, -0.3, -0.2, -0.1, 0.6, 0.7, 0.8, 0.3, 0.4, 0.5};
            int64_t size = aitisa_tensor_size(input);
            for (int64_t i = 0; i < size; i++)
            {
                /* Due to the problem of precision, consider the two numbers
