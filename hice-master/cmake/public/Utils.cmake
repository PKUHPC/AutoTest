if(hice_public_utils_cmake_included)
  return()
endif()
set(hice_public_utils_cmake_included true)

##############################################################################
# Macro to update cached options.
##############################################################################
macro (hice_update_option variable value)
...
##############################################################################
function(hice_make_include_dir target)
...
##############################################################################
function(hice_encode_rpath target rel_intall_libdir)
  if(APPLE)
...
##############################################################################
function(hice_maybe_configure_windows_test name kind)
    if(WIN32 OR MINGW)
...
##############################################################################
function(hice_register_exe name srcs test)
    add_executable(${name} ${srcs})
    target_link_libraries(${name} ${ARGN})
    if("${test}" STREQUAL "test")
        add_test(NAME ${name} COMMAND $<TARGET_FILE:${name}>)
        hice_maybe_configure_windows_test(${name} TEST)
    endif()
endfunction()
...
#   srcs -- list of source, if many must be enclosed with ""
##############################################################################
function(hice_register_example name srcs hice_install_dir)
  add_executable(${name} ${srcs})
  find_package(hice CONFIG REQUIRED HINTS "${hice_install_dir}/cmake")
  target_link_libraries(${name} hice)
endfunction()
...
# Taken from PyTorch
##############################################################################
macro(hice_make_interface_library src dst)
...
# Usage:
#   hice_option(<option_variable> "doc string" <initial value> [dependencies other_value])
# Taken from mxnet
########################################################################################################
function(hice_option variable description value)
  if(NOT ARGN)
