// This file is based on aten\src\Aten\cpu\vec256\vec256_double.h from PyTorch.
...
class Vec256<double> {
 ...
 public:
  ...
  Vec256(double val) { values = _mm256_set1_pd(val); }
  ...
  static Vec256<double> blend(const Vec256<double>& a,
  ...
  static Vec256<double> loadu(const void* ptr, int64_t count = size()) {
    ...
    __hice_align32__ double tmp_values[size()];
    std::memcpy(tmp_values, reinterpret_cast<const double*>(ptr),
                count * sizeof(double));
  ...
  Vec256<double> erf() const { return Vec256<double>(Sleef_erfd4_u10(values)); }
  Vec256<double> erfc() const {
    return Vec256<double>(Sleef_erfcd4_u15(values));
  }
  Vec256<double> exp() const { return Vec256<double>(Sleef_expd4_u10(values)); }
  Vec256<double> expm1() const {
    return Vec256<double>(Sleef_expm1d4_u10(values));
  }
  Vec256<double> log() const { return Vec256<double>(Sleef_logd4_u10(values)); }
  ...
  Vec256<double> sin() const { return map(std::sin); }
  Vec256<double> sinh() const { return map(std::sinh); }
  Vec256<double> cos() const { return map(std::cos); }
  Vec256<double> cosh() const { return map(std::cosh); }
  Vec256<double> ceil() const { return _mm256_ceil_pd(values); }
  Vec256<double> floor() const { return _mm256_floor_pd(values); }
  Vec256<double> frac() const;
  Vec256<double> neg() const {
    return _mm256_xor_pd(_mm256_set1_pd(-0.), values);
  }
  Vec256<double> round() const {
    return _mm256_round_pd(values,
                           (_MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC));
  }
  Vec256<double> tan() const { return map(std::tan); }
  ...
  Vec256<double> sqrt() const { return _mm256_sqrt_pd(values); }
...
Vec256<double> Vec256<double>::frac() const { return *this - this->trunc(); }
...
#ifdef __AVX2__
template <>
Vec256<double> inline fmadd(const Vec256<double>& a, const Vec256<double>& b,
