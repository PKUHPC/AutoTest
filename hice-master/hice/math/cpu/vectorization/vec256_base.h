// This file is based on aten\src\Aten\cpu\vec256\vec256_base.h from PyTorch.
...
#if defined(__GNUC__)
#define __hice_align32__ __attribute__((aligned(32)))
#elif defined(_WIN32)
#define __hice_align32__ __declspec(align(32))
#else
#define __hice_align32__
#endif
...
struct Vec256 {
 ...
 public:
  ...
  template <typename... Args,
            typename = hice::ext::enable_if_t<(sizeof...(Args) == size())>>
  Vec256(Args... vals) {
    values = {vals...};
  }
  ...
  static Vec256<T> arange(T base = static_cast<T>(0),
  ...
  static Vec256<T> set(const Vec256<T>& a, const Vec256<T>& b,
                       int64_t count = size()) {
    Vec256 vec;
    for (int64_t i = 0; i < size(); i++) {
      if (i < count) {
  ...
  Vec256<T> acos() const { return map(std::acos); }
  Vec256<T> asin() const { return map(std::asin); }
  Vec256<T> atan() const { return map(std::atan); }
  ...
  Vec256<T> erf() const { return map(std::erf); }
  Vec256<T> erfc() const { return map(std::erfc); }
  Vec256<T> exp() const { return map(std::exp); }
  Vec256<T> expm1() const { return map(std::expm1); }
  Vec256<T> frac() const { return *this - this->trunc(); }
  Vec256<T> log() const { return map(std::log); }
  Vec256<T> log10() const { return map(std::log10); }
  Vec256<T> log1p() const { return map(std::log1p); }
  Vec256<T> log2() const { return map(std::log2); }
  Vec256<T> ceil() const { return map(std::ceil); }
  Vec256<T> cos() const { return map(std::cos); }
  Vec256<T> cosh() const { return map(std::cosh); }
  Vec256<T> floor() const { return map(std::floor); }
  Vec256<T> neg() const {
  ...
  Vec256<T> round() const { return map(std::nearbyint); }
  Vec256<T> sin() const { return map(std::sin); }
  Vec256<T> sinh() const { return map(std::sinh); }
  Vec256<T> tan() const { return map(std::tan); }
  Vec256<T> tanh() const { return map(std::tanh); }
  Vec256<T> trunc() const { return map(std::trunc); }
  Vec256<T> sqrt() const { return map(std::sqrt); }
...
hice::ext::enable_if_t<scale == 1 || scale == 2 || scale == 4 || scale == 8,
                  Vec256<T>> inline gather(T const* base_addr,
                                           const Vec256<int_same_size_t<T>>&
                                               vindex) {
...
hice::ext::enable_if_t<
    ...
    Vec256<T>> inline mask_gather(const Vec256<T>& src, T const* base_addr,
...
template <typename T>
inline hice::ext::enable_if_t<Vec256<T>::size() % 2 == 0,
                         std::pair<Vec256<T>, Vec256<T>>>
...
#pragma unroll
  ...
  for (int64_t i = 0; i < n; i++) {
    *dst =
        static_cast<dst_T>(static_cast<hice::inter_copy_type_t<dst_T>>(*src));
    src++;
    dst++;
  }
}
