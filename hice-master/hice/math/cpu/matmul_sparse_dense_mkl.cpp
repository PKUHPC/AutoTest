...
void spmv(const Tensor &tensor1, const Tensor &tensor2, Tensor &result,
  ...
  switch (sc_type) {
    case ScalarType::Float: {
      ...
      scalar_t alpha = 1.0;
      scalar_t beta = 0.0;
      HICE_MKLSPARSE_CHECK(mkl_sparse_s_mv(trans, alpha, cooA, descrA, x, beta, y));
      break;
    }
    case ScalarType::Double: {
      ...
      scalar_t alpha = 1.0;
      scalar_t beta = 0.0;
      HICE_MKLSPARSE_CHECK(mkl_sparse_d_mv(trans, alpha, cooA, descrA, x, beta, y));
      break;
    }
    default:
...
void spmm(const Tensor &tensor1, const Tensor &tensor2, Tensor &result,
  ...
  switch (sc_type) {
    case ScalarType::Float: {
      ...
      scalar_t alpha = 1.0;
      scalar_t beta = 0.0;
      HICE_MKLSPARSE_CHECK(mkl_sparse_s_mm(trans_a, alpha, cooA, descrA, SPARSE_LAYOUT_ROW_MAJOR, x,
    ...
    case ScalarType::Double: {
      ...
      scalar_t alpha = 1.0;
      scalar_t beta = 0.0;
      HICE_MKLSPARSE_CHECK(mkl_sparse_d_mm(trans_a, alpha, cooA, descrA, SPARSE_LAYOUT_ROW_MAJOR, x,
                      n_cols_result, ldx, beta, y, ldy));
  ...
  HICE_MKLSPARSE_CHECK(mkl_sparse_destroy(cooA));
}
...
void matmul_impl(const Tensor &tensor1, const Tensor &tensor2, Tensor &result,
  ...
  // std::cout << "Kernel: matmul_cpu_sparse_dense" << std::endl;
