...
void spmv(const Tensor &tensor1, const Tensor &tensor2, Tensor &result,
  ...
  const int *colind_csr = tensor1.column_indices().data<int>();
  const int *rowind_csr = tensor1.row_offsets().data<int>();
  ...
  switch (sc_type) {
    case ScalarType::Float: {
      ...
      cusparseScsrmv(handle, trans, n_rows, n_cols, n_nonzero, &alpha, descr,
                     val_ptr, rowind_csr, colind_csr, x, &beta, y);
      break;
    }
  ...
  switch (sc_type) {
    case ScalarType::Float: {
      ...
      cusparseScsrmm2(cusparse_handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
        ...
        nnzA, &alpha, descr, valuesA, rowindA_csr, colindA_csr,
        B, ldb, &beta, Ct, ldc);
      // convert Ct to C.
...
void matmul_impl(const Tensor &tensor1_, const Tensor &tensor2, Tensor &result,
  ...
  if (ndim_tensor1 == 2 && ndim_tensor2 == 1) {
    ...
    HICE_CHECK(false) << "For csr-dense matmul, Only mv and mm is "
