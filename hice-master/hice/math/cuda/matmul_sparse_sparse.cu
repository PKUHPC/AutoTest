...
void spgemm(const Tensor &tensor1, const Tensor &tensor2, Tensor &result) {
  ...
  Tensor indicesA = tensor1.indices();
  Tensor indicesB = tensor2.indices();
  ...
  cudaMalloc(&rowindC_csr, sizeof(int) * (m + 1));
  ...
  HICE_CUSPARSE_CHECK(cusparseXcsrgemmNnz(
    ...
    CUSPARSE_OPERATION_NON_TRANSPOSE,
    m, n, k,
    descr, nnzA, rowindA_csr, colindA,
    descr, nnzB, rowindB_csr, colindB,
    descr, rowindC_csr, &nnzC));
  // resize
  result.resize_with_nnz(nnzC);
  int *indicesC = result.mutable_indices().mutable_data<int32_t>();
  int *rowindC_coo = indicesC;
  int *colindC = indicesC + nnzC;
  ...
  switch (sc_type) {
    case ScalarType::Float: {
      ...
      HICE_CUSPARSE_CHECK(cusparseScsrgemm(
        ...
        CUSPARSE_OPERATION_NON_TRANSPOSE,
        m, n, k,
        descr, nnzA, valuesA, rowindA_csr, colindA,
        descr, nnzB, valuesB, rowindB_csr, colindB,
        descr, valuesC, rowindC_csr, colindC));
      break;
    }
  ...
  // convert csr to coo
  HICE_CUSPARSE_CHECK(cusparseXcsr2coo(handle, rowindC_csr, nnzC, 
                                       m, rowindC_coo, 
                                       CUSPARSE_INDEX_BASE_ZERO));
