// This file is based on c10\core\Stream.h from PyTorch. 
...
public:
  ...
  static Stream unpack(uint64_t bits) {
    ...
    HICE_CHECK(is_valid_device_type(device_type));
...
namespace std {
  template <>
  struct hash<hice::Stream> {
    size_t operator()(hice::Stream s) const noexcept {
