// This file is based on c10\core\impl\InlineDeviceGuard.h from PyTorch. 
...
#include "hice/core/impl/virtual_guard_impl.h"
 ...
 *    InlineDeviceGuard this way as hice::DeviceGuard.
 *
...
public:
  ...
  explicit InlineDeviceGuard(Device device)
    : impl_(device.type())
    , original_device_(device.index() == -1 ? impl_.get_device() : impl_.exchange_device(device))
  ...
  explicit InlineDeviceGuard(Device device, const DeviceGuardImplInterface* impl)
    : impl_(VirtualGuardImpl(impl ? impl : get_device_guard_impl(device.type())))
    , original_device_(device.index() == -1 ? impl_.get_device() : impl_.exchange_device(device))
    , current_device_(device.index() == -1 ? original_device_ : device)
    {}
  ...
  template <typename U=T>
  typename std::enable_if<std::is_same<U, VirtualGuardImpl>::value >::type
  reset_device(Device device, const impl::DeviceGuardImplInterface* impl = nullptr) {
    auto index = device.index();
    if (index == -1) return;
    if (device.type() == original_device_.type()) {
      HICE_CHECK(impl == nullptr || impl->type() == device.type());
      impl_.set_device(device);
      current_device_ = device;
    } else {
      // Destruct and reconstruct the DeviceGuard in place
      impl_.set_device(original_device_);
      impl_ = !impl ? VirtualGuardImpl(device.type()) : VirtualGuardImpl(impl);
      original_device_ = impl_.exchange_device(device);
      current_device_ = device;
    }
  }
...
public:
  ...
  // Tensor moves to hice.  (These are only valid on OptionalDeviceGuard,
  ...
  /// Optional argument is for testing only.
  template <typename U=T, typename=typename std::enable_if<std::is_same<U, VirtualGuardImpl>::value>::type>
  void reset_device(Device device, const DeviceGuardImplInterface* impl = nullptr) {
    if (!guard_.has_value()) {
