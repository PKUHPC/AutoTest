// This file is based on c10\core\impl\InlineStreamGuard.h from PyTorch. 
...
#include "hice/core/impl/inline_device_guard.h"
...
public:
  ...
  explicit InlineStreamGuard(Stream stream)
    ...
    , original_stream_of_original_device_(this->impl_.get_stream(original_device()))
    , original_stream_of_current_device_(this->impl_.exchange_stream(stream))
  ...
  explicit InlineStreamGuard(Stream stream, const DeviceGuardImplInterface* impl)
    : InlineDeviceGuard<T>(stream.device(), impl ? impl : get_device_guard_impl(stream.device_type()))
    , original_stream_of_original_device_(this->impl_.get_stream(original_device()))
    , original_stream_of_current_device_(this->impl_.exchange_stream(stream))
    , current_stream_(stream)
    {}
  ...
  ~InlineStreamGuard() {
    this->impl_.exchange_stream(original_stream_of_current_device_);
  }
  ...
  void reset_stream(Stream stream) {
    ...
    if (stream.device() == this->current_device()) {
      this->impl_.exchange_stream(stream);
      current_stream_ = stream;
    } else {
