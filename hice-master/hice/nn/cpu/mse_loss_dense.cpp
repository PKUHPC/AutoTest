...
void mse_loss_fwd_impl(const Tensor &input, const Tensor &target,
...
void mse_loss_bwd_impl(const Tensor &input, const Tensor &target,
                       hice::optional<Tensor> weight, Reduction reduction,
                       const Tensor &grad_loss, Tensor &grad_input) {
  Tensor tmp = sub(input, target);
  square_bwd(tmp, grad_loss, grad_input);
  if (weight) {
...
// Forward
HICE_REGISTER_KERNEL(mse_loss_fwd_dispatcher, &mse_loss_fwd_impl,
                     {kCPU, kDense},  // input
                     {kCPU, kDense},  // target
                     {kCPU, kDense}   // loss
);
...
// Backward
HICE_REGISTER_KERNEL(mse_loss_bwd_dispatcher, &mse_loss_bwd_impl,
                     {kCPU, kDense},  // input
                     {kCPU, kDense},  // target
