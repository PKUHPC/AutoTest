...
//    static constexpr extent_type extent = Extent;
    ...
    // HICE extension
    template <typename U>
    span(const hice::SmallVectorTemplateCommon<T, U>& vec)
        : data_(const_cast<T*>(vec.data())), size_(vec.size()) {}
    
    span_constexpr span(const std::initializer_list<T> &vec)
        : data_(vec.begin() == vec.end() ? static_cast<T*>(nullptr) : const_cast<T*>(vec.begin())),
          size_(vec.size()) {}
