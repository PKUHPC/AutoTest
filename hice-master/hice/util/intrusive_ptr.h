// This file is based on c10\util\intrusive_ptr.h from PyTorch. 
...
#  pragma GCC diagnostic pop
  ...
  size_t ref_count() const { return refcount_.load(); }

  size_t weak_count() const { return weakcount_.load(); }
...
//      "intrusive_ptr can only be used for classes that inherit from intrusive_ptr_target.");
  ...
  void reset_() noexcept {
    if (target_ != NullType::singleton() && --target_->refcount_ == 0) {
      ...
      const_cast<hice::ext::remove_const_t<TTarget>*>(target_)->release_resources();
      if (weak_count == 0) {
        delete target_;
      }
    }
 ...
 public:
  ...
  static intrusive_ptr reclaim(TTarget* owning_ptr) {
    ...
    HICE_CHECK(owning_ptr == NullType::singleton() ||
               owning_ptr->refcount_.load() > 0)
        << "intrusive_ptr: Can only intrusive_ptr::reclaim() owning pointers "
           "that were created using intrusive_ptr::release().";
  ...
  static intrusive_ptr unsafe_reclaim_from_nonowning(TTarget* raw_ptr) {
    // See Note [Stack allocated intrusive_ptr_target safety]
    HICE_CHECK(raw_ptr == NullType::singleton() ||
...
class weak_intrusive_ptr final {
 private:
  ...
  void retain_() {
    if (target_ != NullType::singleton()) {
      ...
      HICE_CHECK(new_weakcount != 1) << "weak_intrusive_ptr: Cannot increase "
                                        "weakcount after it reached zero.";
    }
  }
 ...
 public:
  ...
  static weak_intrusive_ptr reclaim(TTarget* owning_weak_ptr) {
    ...
    HICE_CHECK(owning_weak_ptr == NullType::singleton() ||
...
namespace intrusive_ptr {
  ...
  inline void incref(intrusive_ptr_target* self) {
    auto ptr = hice::intrusive_ptr<intrusive_ptr_target>::reclaim(self);
    auto ptr_copy = ptr;
    ptr_copy.release();
    ptr.release();
  }
  ...
  inline void decref(intrusive_ptr_target* self) {
    // Let it die
    hice::intrusive_ptr<intrusive_ptr_target>::reclaim(self);
    // NB: Caller still has 'self' pointer, but it's now invalid.
    // If you want more safety, used the actual hice::intrusive_ptr class
  }

  template <typename T>
  inline T* make_weak(T* self) {
    // NB: 'this' is a strong pointer, but we return a weak pointer
    auto ptr = hice::intrusive_ptr<T>::reclaim(self);
    hice::weak_intrusive_ptr<T> wptr(ptr);
    ptr.release();
    return wptr.release();
  }
...
namespace weak_intrusive_ptr {
  ...
  inline void incref(weak_intrusive_ptr_target* self) {
    auto wptr = hice::weak_intrusive_ptr<weak_intrusive_ptr_target>::reclaim(self);
    auto wptr_copy = wptr;
    wptr_copy.release();
    wptr.release();
  }
  ...
  inline void decref(weak_intrusive_ptr_target* self) {
    // Let it die
    hice::weak_intrusive_ptr<intrusive_ptr_target>::reclaim(self);
    // NB: You still "have" the 'self' pointer, but it's now invalid.
    // If you want more safety, used the actual hice::weak_intrusive_ptr class
  }

  template <typename T>
  inline T* lock(T* self) {
    auto wptr = hice::weak_intrusive_ptr<T>::reclaim(self);
    auto ptr = wptr.lock();
    wptr.release();
    return ptr.release();
  }
  ...
  inline uint32_t use_count(weak_intrusive_ptr_target* self) {
    auto wptr = hice::weak_intrusive_ptr<intrusive_ptr_target>::reclaim(self);
...
template <class TTarget, class NullType>
struct hash<hice::intrusive_ptr<TTarget, NullType>> {
  size_t operator()(const hice::intrusive_ptr<TTarget, NullType>& x) const {
    return std::hash<TTarget*>()(x.get());
  }
};
template <class TTarget, class NullType>
struct hash<hice::weak_intrusive_ptr<TTarget, NullType>> {
  size_t operator()(const hice::weak_intrusive_ptr<TTarget, NullType>& x) const {
