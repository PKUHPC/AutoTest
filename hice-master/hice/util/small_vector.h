...
// replaced isPodLike<T> with IS_TRIVIALLY_COPYABLE (moved to Macros.h)
...
// HICE: slightly modified from ATen 
...
class HICE_API SmallVectorBase {
 protected:
...
class SmallVectorImpl
    : public SmallVectorTemplateBase<T, IS_TRIVIALLY_COPYABLE(T)> {
  using SuperClass = SmallVectorTemplateBase<T, IS_TRIVIALLY_COPYABLE(T)>;
 ...
 protected:
  ...
  explicit SmallVectorImpl(unsigned N)
      : SmallVectorTemplateBase<T, IS_TRIVIALLY_COPYABLE(T)>(N * sizeof(T)) {
  }
...
template <typename T>
inline void swap(hice::SmallVectorImpl<T>& LHS, hice::SmallVectorImpl<T>& RHS) {
  LHS.swap(RHS);
}

/// Implement std::swap in terms of SmallVector swap.
template <typename T, unsigned N>
inline void swap(hice::SmallVector<T, N>& LHS, hice::SmallVector<T, N>& RHS) {
  LHS.swap(RHS);
}
