...
#include "hice/core/export.h"
	...
	#define LOGURU_EXPORT HICE_API
#endif
#define LOGURU_REPLACE_GLOG 1

// See [NOTE ABOUT LOG] in project/CMakeLists.txt
#ifndef HICE_USE_THROW_EXCEPTION
	#ifdef NDEBUG	
		#define HICE_USE_THROW_EXCEPTION 1
	#else
		#define HICE_USE_THROW_EXCEPTION 0 
	#endif
#endif
...
#define HICE_LOG_PREFIX 1
#if HICE_LOG_PREFIX

	// NOTE: LOG() is only for developers, its outputs are invisible in release version.
	// Use CHECK_X() if you want to tell user something when error occurred.
	#define HICE_VLOG_IF(verbosity, cond)				VLOG_IF_S(verbosity, cond)	
	#define HICE_LOG_IF(verbosity_name, cond)		LOG_IF_S(verbosity_name, cond)	
	#define HICE_VLOG(verbosity)								VLOG_S(verbosity)	
	#define HICE_LOG(verbosity_name)						LOG_S(verbosity_name)	
	#define HICE_DLOG(verbosity_name) 					DLOG_S(verbosity_name)	
	#define HICE_DVLOG(verbosity) 							DVLOG_S(verbosity)	
	#define HICE_DLOG_IF(verbosity_name, cond)	DLOG_IF_S(verbosity_name, cond)	
	#define HICE_DVLOG_IF(verbosity, cond) 			DVLOG_IF_S(verbosity, cond)	

	#define HICE_CHECK(cond)										CHECK_S(!!(cond))	
	#define HICE_CHECK_NOTNULL(x) 							CHECK_NOTNULL_S(x)	
	#define HICE_CHECK_EQ(expr1, expr2) 				CHECK_EQ_S(expr1, expr2)	
	#define HICE_CHECK_NE(expr1, expr2) 				CHECK_NE_S(expr1, expr2)	
	#define HICE_CHECK_LT(expr1, expr2) 				CHECK_LT_S(expr1, expr2)	
	#define HICE_CHECK_LE(expr1, expr2) 				CHECK_LE_S(expr1, expr2)	
	#define HICE_CHECK_GT(expr1, expr2) 				CHECK_GT_S(expr1, expr2)	
	#define HICE_CHECK_GE(expr1, expr2) 				CHECK_GE_S(expr1, expr2)	
	#define HICE_DCHECK(cond) 									DCHECK_S(cond)	
	#define HICE_DCHECK_NOTNULL(x)							DCHECK_NOTNULL_S(x)	
	#define HICE_DCHECK_EQ(a, b) 								DCHECK_EQ_S(a, b)	
	#define HICE_DCHECK_NE(a, b) 								DCHECK_NE_S(a, b)
	#define HICE_DCHECK_LT(a, b) 								DCHECK_LT_S(a, b)
	#define HICE_DCHECK_LE(a, b) 								DCHECK_LE_S(a, b)
	#define HICE_DCHECK_GT(a, b) 								DCHECK_GT_S(a, b)
	#define HICE_DCHECK_GE(a, b) 								DCHECK_GE_S(a, b)

	#define HICE_CHECK_TYPE_MATCH(cond) 		CHECK_S(!!(cond)) << " {Hice::TypeMismatch} "
	#define HICE_CHECK_DIMS_MATCH(cond) 		CHECK_S(!!(cond)) << " {Hice::kDimensionsMismatch} "
	#define HICE_CHECK_ARGUMENT(cond) 			CHECK_S(!!(cond)) << " {Hice::kInvalidArgument} "
	#define HICE_CHECK_ALLOC(cond) 					CHECK_S(!!(cond)) << " {Hice::kAllocFailed} "
	#define HICE_CHECK_RANGE(cond) 					CHECK_S(!!(cond)) << " {Hice::kOutOfRange} "
	#define HICE_CHECK_INTERNAL(cond) 			CHECK_S(!!(cond)) << " {Hice::kInternalError} "
	#define HICE_CHECK_MATH(cond)					 	CHECK_S(!!(cond)) << " {Hice::kMathError} "
	#define HICE_CHECK_SUPPORTED(cond) 			CHECK_S(!!(cond)) << " {Hice::kNotSupported} "

	#define HICE_VLOG_IS_ON(verbosity) ((verbosity) <= loguru::current_verbosity_cutoff())
#endif // HICE_LOG_PREFIX
