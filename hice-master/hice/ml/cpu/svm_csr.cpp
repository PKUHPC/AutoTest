...
namespace{
void svm_cpu_csr(const Tensor& train_data, const Tensor& label,
         const Tensor& predict_data, Tensor &result,
         SvmParam param){
  //  std::cout << "=========Kernel: svm_cpu_csr==========" << std::endl;
   ...
   Tensor working_set_last_half = wrap(
       ...
       device(kCPU).dtype(kInt32), false);
   Tensor self_dot_csr = full({num_vects}, 0, device(kCPU).dtype(kFloat));
   Tensor data_row_nnz = full({num_vects}, 0, device(kCPU).dtype(kInt32));
   get_row_nnz_cpu(train_data, data_row_nnz, num_vects);
   // tp.print(data_row_nnz);
   get_self_dot_csr_cpu(train_data, self_dot_csr, data_row_nnz);
   // tp.print(self_dot_csr);
   Tensor data_rows = full({ws_size, dim_vects}, 0, device(kCPU).dtype(kFloat));
   Tensor data_rows_half = full({half_ws_size, dim_vects}, 0, device(kCPU).dtype(kFloat));
   ...
   const int *w_s_f_h = working_set_first_half.data<int>();
  ...
  //  std::cout << "traing start" << std::endl;
   for (int iter = 0;; ++iter) // global _ iter
   {
     ...
     if (0 == iter) {
       ...
       // get_working_set_from_csr_cpu(train_data, working_set, data_rows, ws_size, dim_vects);
       get_working_set_from_csr_row_major_cpu(train_data, working_set, data_rows, ws_size, dim_vects);
       svm_matmul_dense_csr_cpu(train_data, data_rows, k_mat_rows);
       if(param.kernel_type == SvmParam::RBF){
         RBF_kernel_cpu(working_set, self_dot_csr, k_mat_rows, ws_size, num_vects, gamma);
     ...
     #pragma omp parallel for schedule(guided)
       ...
       // get_working_set_from_csr_cpu(train_data, working_set_last_half, data_rows_half,
                           // half_ws_size, dim_vects);
       get_working_set_from_csr_row_major_cpu(train_data, working_set_last_half, data_rows_half,
                                              half_ws_size, dim_vects);
       svm_matmul_dense_csr_cpu(train_data, data_rows_half, k_mat_rows_last_half);
       // tp.print(k_mat_rows);
       if(param.kernel_type == SvmParam::RBF){
         RBF_kernel_cpu(working_set_last_half, self_dot_csr, k_mat_rows_last_half,
  ...
  //  std::cout << "predict start" << std::endl;
   ...
   get_working_set_from_csr_row_major_cpu(train_data, sv_id, sv_matrix, sv_size, dim_vects);
   Tensor predict_k_mat_rows_T = full({sv_size, predict_num_vects}, 0, device(kCPU).dtype(kFloat));
   Tensor predict_k_mat_rows = full({predict_num_vects, sv_size}, 0, device(kCPU).dtype(kFloat));
   svm_matmul_dense_csr_cpu(predict_data, sv_matrix, predict_k_mat_rows_T);
   ...
   Tensor data_row_nnz_predict = full({predict_num_vects}, 0, device(kCPU).dtype(kInt32));
   Tensor self_dot_predict = full({predict_num_vects}, 0, device(kCPU).dtype(kFloat));
   get_row_nnz_cpu(predict_data, data_row_nnz_predict, predict_num_vects);
   get_self_dot_csr_cpu(predict_data, self_dot_predict, data_row_nnz_predict);
   if(param.kernel_type == SvmParam::RBF){
...
void svm_impl(const Tensor& train_data, const Tensor& label,
         const Tensor& predict_data, Tensor &result,
         SvmParam param){
    svm_cpu_csr(train_data, label,predict_data, result, param);
} // function
}
HICE_REGISTER_KERNEL(svm_dispatcher, &svm_impl,
                     {kCPU, kCSR}, // train_data
                     {kCPU, kDense}, // label
                     {kCPU, kCSR}, // predict_data
                     {kCPU, kDense}, // result
);
